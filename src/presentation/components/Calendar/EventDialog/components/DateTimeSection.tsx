      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 max-w-md w-full mx-4 shadow-xl">
        ...
        <div className="p-6 space-y-6">
          <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
            {type === "overdue"
              ? `The end time you selected (${formatDateTime(
                  dueDate,
                  dueTime
                )}) has already passed. Please choose an appropriate action:`
              : "End date/time cannot be earlier than start date/time. Please adjust the times."}
          </p>
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    Keep as scheduled but mark as overdue
                  </div>
                </div>
              </button>
            </div>
...
const DateTimeSection: React.FC<DateTimeSectionProps> = ({
  ...
  useEffect(() => {
    if (!editedTask.timeZone) {
      handleChange("timeZone", getCurrentTimeZone());
    }
  }, [editedTask.timeZone, handleChange]);
  ...
  const handleDueDateChange = useCallback(
    (newDueDate: Date | null) => {
      if (!newDueDate) {
        ...
        // Update legacy field
        const legacyEnd = combineDateTime(null, editedTask.dueTime);
        handleChange("end", legacyEnd);
        return;
      }
      ...
      if (
        ...
        // Update legacy field
        const legacyEnd = combineDateTime(corrected.dueDate, corrected.dueTime);
        handleChange("end", legacyEnd);
        return;
      }
      ...
      // Update legacy field
      const legacyEnd = combineDateTime(newDueDate, editedTask.dueTime);
      handleChange("end", legacyEnd);
    },
  ...
  const handleDueTimeChange = useCallback(
    (newDueTime: Date | null) => {
      ...
      if (corrected.dueTime !== newDueTime) {
        handleChange("dueTime", corrected.dueTime);
        // Update legacy field
        const legacyEnd = combineDateTime(
          editedTask.dueDate,
          corrected.dueTime
        );
        handleChange("end", legacyEnd);
      ...
      // Update legacy field
      const legacyEnd = combineDateTime(editedTask.dueDate, newDueTime);
      handleChange("end", legacyEnd);
    },
    [editedTask, handleChange, preventInvalidRange]
  );
  ...
  const handleStartDateChange = useCallback(
    (newStartDate: Date | null) => {
      ...
      // Update legacy field
      const legacyStart = combineDateTime(newStartDate, editedTask.startTime);
      handleChange("start", legacyStart);
    },
    [editedTask.startTime, handleChange]
  );
  ...
  const handleStartTimeChange = useCallback(
    (newStartTime: Date | null) => {
      ...
      // Update legacy field
      const legacyStart = combineDateTime(editedTask.startDate, newStartTime);
      handleChange("start", legacyStart);
    },
    [editedTask.startDate, handleChange]
  );
  ...
  const handleValidationComplete = useCallback(() => {
    ...
    // Update legacy field
    const legacyEnd = combineDateTime(
      validationDialog.pendingDueDate ?? null,
      validationDialog.pendingDueTime ?? null
    );
    handleChange("end", legacyEnd);
  ...
  const handleValidationOverdue = useCallback(() => {
    // Apply pending date/time changes but don't mark as completed
    if (validationDialog.pendingDueDate !== undefined) {
      handleChange("dueDate", validationDialog.pendingDueDate ?? null);
    }
    if (validationDialog.pendingDueTime !== undefined) {
  ...
  return (
    <>
      <div className="space-y-6">
        {/* Timezone Display */}
        {editedTask.timeZone && (
          <div className="text-xs text-gray-500 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800 flex items-center gap-2">
            <Globe size={14} className="text-blue-600 dark:text-blue-400" />
            <strong>Timezone:</strong> {formatTimeZone(editedTask.timeZone)}
          </div>
        )}
        ...
        {isStartDateTimeSet && (
          <div className="text-xs text-gray-500 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
            💡 <strong>Note:</strong> End date/time cannot be earlier than start
            date/time.
            {!isCreateMode &&
              " If you set end time in the past, the system will ask about event status when you click 'Done'."}
          </div>
        )}
